...
room_id_grouped <- group_by(airbnb, room_id, longitude, latitude, bedrooms) %>% 
  summarise(n = n())

ggmap(myMap) +
geom_jitter(alpha = 0.5, aes(x=longitude, y=latitude), 
...
When the large outliers are removed, the price data is still right-skewed.

```{r} 
#Remove or move, does not belong here
...
airbnb$price_bin <- percent_rank(airbnb$price) %>% 
  ...
  mutate(bin = plot <- _else(rank > 0.5, 1, 2 ))
...
airbnb %>% 
  group_by(room_id, longitude, latitude) %>% 
  summarise(n = n()) %>% 
  arrange(desc(n)) %>% 
  group_by(room_id) %>% 
  summarise(n = n()) %>% 
...
plot1 <- ggplot(airbnb, aes(y = price)) +
  geom_bar(stat = 'summary', fun.y = mean, 
           aes(x = reorder(neighborhood, price, FUN = mean), fill = I('gray50'))) +
  geom_bar(stat = 'summary', fun.y = median, 
           aes(x = reorder(neighborhood, price),
