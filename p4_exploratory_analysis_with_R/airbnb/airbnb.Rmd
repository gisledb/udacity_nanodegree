...
see union_script.R in the p6 folder.
...
I will start my analysis by taking a look at some records, to get an initial 
feel of the dataset.
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Univariate_Plots01}
#Getting an initital look at some rows
head(airbnb)
```
   
```{r echo=FALSE, message=FALSE, warning=FALSE, Univariate_Plots02}
#Getting an initital look at some rows
tail(airbnb)
```
\
...
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Univariate_Plots03}
str(airbnb)
```
   
```{r warning=FALSE, Univariate_Plots04}
...
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Univariate_Plots05}
summary(airbnb)
```
 
```{r echo=FALSE, message=FALSE, warning=FALSE, Univariate_Plots06}
...
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Univariate_Plots07}
...
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Univariate_Plots08}
airbnb %>% 
...
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Univariate_Plots09}
...
\
\
```{r include=FALSE, Univariate_Plots12}
#For QA
pre_change <- airbnb %>% 
...
p1 = ggplot(filter(airbnb, overall_satisfaction == 0 & reviews != 0), aes(x = reviews, fill = I('#099DD9') ) ) +
  geom_bar() +
  ggtitle("Listings with overall_satisfaction 0") +
  geom_text(stat='count',aes(
    label=scales::percent(..count../sum(..count..)),vjust=-0.5))
...
p2 = ggplot(filter(airbnb, reviews != 0), aes(x = reviews, fill = I('#099DD9') ) ) +
  ...
  xlim(0,50) +
  ggtitle("Listings with 0-50 reviews")
...
\
\
Almost all the listings with overall_satisfaction score 0 have either 1 or 2 
reviews.
...
plot <- ggplot(airbnb, aes(x = bedrooms, fill = I('#099DD9') ) ) +
  geom_bar() +
  scale_x_continuous(breaks = seq(0,max(subset(airbnb,
                                               !is.na(bedrooms))$bedrooms),1)) +
    geom_text(stat='count',aes(
...
more units have a minimum stay of 7 and 30 nights than 6 nights, which is 
natural considering there are 7 days in a week, and (roughly) 30 days in a month. 
There is also a slight increase at 10 days (round number) and 14 days (2 weeks). 
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Univariate_Plots26}
#accommodates
plot <- ggplot(airbnb, aes(x = reorder(neighborhood, 
                                       neighborhood, FUN = length), 
                           fill = I('#099DD9') ) ) +
  geom_bar() + 
  coord_flip()
...
\
\
While it is most common for units to accommodate 2 people, accommodating 4 people 
is also rather common. Few listings accommodate more than 
6 people.
\
...
https://github.com/gisledb/udacity_nanodegree/blob/master/p6_data_visualization_with_Tableau/Project/union_script.R.
...
\
\
```{r fig.height=6, fig.width=6, echo=FALSE, message=FALSE, warning=FALSE, Bivariate_Plots01}

#Fetching map of San Francisco
...
geom_jitter(alpha = 0.01, aes(x=longitude, y=latitude), 
  ...
  scale_x_continuous(limits = c(-122.525, -122.3), 
                     expand = c(0, 0), breaks = NULL) +
  scale_y_continuous(limits = c(37.695, 37.84), 
                     expand = c(0, 0), breaks = NULL) +
  theme(axis.title = element_blank()) +
  ggtitle("Distribution of neighborhood variable")
...
```{r echo=FALSE, message=FALSE, warning=FALSE, Bivariate_Plots03}
airbnb %>% 
...
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Bivariate_Plots04}
...
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Bivariate_Plots05}
...
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Bivariate_Plots06}
plot <- ggplot(airbnb, aes(y = price)) +
  geom_bar(stat = 'summary', fun.y = mean, 
           aes(x = reorder(neighborhood, price, FUN = mean), 
               fill = I('gray50'))) +
  geom_bar(stat = 'summary', fun.y = median, 
           aes(x = reorder(neighborhood, price),
               fill = I('deepskyblue'))) +
...
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Bivariate_Plots07}
...
plot <- ggplot(neighborhood_price, aes(y = (mean / median), 
                                       x = reorder(neighborhood, 
                                                   (mean / median) ) )) +
...
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Bivariate_Plots08}
...
plot <- ggplot(airbnb, aes(y = price, x = reorder(neighborhood,price, FUN = mean))) +
  ...
  ggtitle('Boxplot, price per neighborhood')
...
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Bivariate_Plots09}
...
plot <- ggplot(filter(airbnb, price < quantile(airbnb$price,0.99)), 
  ...
  coord_flip() +
  labs(x = 'Neighborhood', y = 'Price ($)') +
  ggtitle('Boxplot, price per neighborhood', subtitle = 'Top 1% prices excluded')
...
```{r echo=FALSE, message=FALSE, warning=FALSE, Bivariate_Plots15a}
...
```{r echo=FALSE, message=FALSE, warning=FALSE, Bivariate_Plots15b}
...
\
\
Above I am mostly interested in comparing quantiles for price and 
...
plot <- ggplot(airbnb, aes(x = date_collected, 
                           group = date_collected, y = price)) +
  geom_boxplot(outlier.size = 0.5, outlier.alpha = 0.3) + 
  scale_x_date(date_labels = "%b-%y", date_breaks = '6 months') +
  ylab('Price ($)') +
  ggtitle('Boxplot, price over time')
...
\
\
Looking at the three line graphs above, we see that the August 2015 date does 
not have exceptionally many records with price higher than the top 5% prices in 
the whole dataset, but the August 2015 records do have a very large amount of 
the top 2% of prices in the dataset, and almost three times as many of the 1% 
top prices as any other date in the dataset. Since this is showing absolute 
counts, and some of the later dates have more records than the August 2015 date,
...
plot <- ggplot(filter(airbnb, overall_satisfaction == 0), 
               aes(x = factor(date_collected))) +
  geom_bar()
...
\
\
Not a single record before 2016-12-23 has an overall_score of 0. Since there is 
...
plot <- ggplot(filter(airbnb, bedrooms < quantile(airbnb$bedrooms, 
                                                  0.995, na.rm = TRUE)), 
...
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Bivariate_Plots32}

#Plotting price across accomodates
plot <- ggplot(filter(airbnb, accommodates < quantile(airbnb$accommodates, 
                                                      0.995, na.rm = TRUE)), 
       aes(x = accommodates, y = price)) +
  geom_line(stat = 'summary', fun.y = mean, aes(color = I('blue'))) +
  geom_line(stat = 'summary', fun.y = median,  aes(color = I('forestgreen'))) +
  theme_light() +
  labs(caption = 'Excluding top 0.5% accomodates counts.', y = 'Price ($)')
...
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Bivariate_Plots34}
...
information.
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Bivariate_Plots35}
airbnb %>% 
  filter(room_type == '') %>% 
  group_by(date_collected) %>% 
  summarise(n = n())
...
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Bivariate_Plots36}
...
between price and the number of bedrooms and people a unit can accommodate. 
...
and price and people a unit can accommodate. Both accommodation count and number 
...
#Histogram for price
p1 <- plot +
  geom_histogram(binwidth=10, aes(x = price, fill = I('gray50'))) +
  xlim(0,quantile(airbnb$price, 0.95)) +
  xlab('Price ($)')

#Histogram for price_per_bedroom
p2 <- plot +
  geom_histogram(binwidth=10, aes(x = price_per_bedroom, 
                                  fill = I('deepskyblue'))) +
...
plot <- ggplot(mutated, aes(x=date_collected, color = room_type)) +
  ...
  geom_line(aes(y = median_price), linetype = 'longdash', size = 0.5, 
            show.legend = TRUE) +
...
Private rooms first have become a little bit cheaper, while prices for shared 
rooms, excluding one date in August 2015, have been fairly stable.
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Multivariate_Plots03}
#Creating filter for plotting, excluding top 1% room counts.
filtered = filter(airbnb, bedrooms <= quantile(airbnb$bedrooms, 0.99, 
                                               na.rm = TRUE))
...
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Multivariate_Plots04}
...
\
\
```{r fig.height = 8, fig.width = 8, echo=FALSE, message=FALSE, warning=FALSE, Multivariate_Plots05}
plot <- ggplot(airbnb, aes(x = date_collected)) +
  geom_line(stat = 'summary', fun.y = median, 
...
\
\
Looking at this, Presidio stands out. While price per bedroom is fairly stable 
throughout the time period, the unit price varies drastically. Let's take a 
closer look at Presidio to figure out what is going on. 
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Multivariate_Plots06}
Presidio <-
  airbnb %>% 
  subset(neighborhood == 'Presidio') %>% 
  group_by(date_collected) %>% 
  summarise(n = n(), mean = mean(price), median = median(price), 
            min = min(price), max = max(price), mean_bedrooms = mean(bedrooms))

Presidio
```
\
\
Looking at the summary statistics, there seem to be a clear problem with 
drawing any conclusions from the Presidio data: the sample size per date is 
just too small, with most dates having less than 10 records. Let's take a look 
at the sample sizes for all neighborhoods.
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Multivariate_Plots07}
neighborhood_count <-  
  airbnb %>% 
  group_by(neighborhood) %>% 
  summarise(n = n()) %>% 
...
nearly 60% fewer records than the second lowest neighborhood. However, some of 
the other neightborhoods might also have too few records to be statistically 
viable. Let's take a closer look at the records with less than 1000 total 
records.
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Multivariate_Plots08}

filter_values <- filter(neighborhood_count, n <=1000)$neighborhood

ggplot(filter(airbnb, neighborhood %in% filter_values), 
...
\
\
To some extent all of these are troubling. With the exception of Crocker 
Amazon, all these neighborhoods have some dates with less than 10 records. 
However, only the Presidio has consistently around 10 or fewer records per date
over time.
\
\
```{r fig.height = 8, fig.width = 8, echo=FALSE, message=FALSE, warning=FALSE, Multivariate_Plot}
plot <- ggplot(filter(airbnb, !(neighborhood %in% filter_values)), 
       aes(x = date_collected)) +
  geom_line(stat = 'summary', fun.y = median, 
            aes(y = price, color = I('blue'))) +
  geom_line(stat = 'summary', fun.y = median, 
...
\
\
Excluding the low-record neighborhoods shows a clearer relationship between 
median price and median price per bedroom for all neighborhoods. 
I also notice that except for the first date, which we know has many fewer 
records compared to the other dates, the median price per bedroom within 
neighborhoods stayed fairly stable throughout the time period. The same is true 
for median price, except for Presidio Heights, which had some peaks on certain 
dates.
\
\
```{r echo=FALSE, message=FALSE, warning=FALSE, Multivariate_Plots09}
...
```{r fig.height=6, fig.width=6, echo=FALSE, message=FALSE, warning=FALSE, Multivariate_Plots10}
...
plot <- ggmap(myMap) +
geom_jitter(alpha = 0.1, size = 0.1, aes(x=longitude, y=latitude), 
           data= filter(room_id_grouped, !is.na(price_bin))) +
  ggtitle("Location of listings per price quartiles") +
...
neighborhood, and prices within neighborhoods stayed fairly stable throughout 
the date range.

### Were there any interesting or surprising interactions between features?

I found it interesting how a relatively large dataset can still be too small for 
detailed analysis when you include multiple variables. The clearest example of 
this was how I had to exclude certain neighborhoods from my analysis due to 
having too few data points (less than 10) on certain dates.
...
myMap <- get_map(location = c(lon= -122.431297,lat = 37.773972), 
                 source = "stamen", 
                 maptype = 'toner-lite', crop = FALSE, zoom = 12)

plot3 <- ggmap(myMap) +
geom_jitter(alpha = 0.1, size = 0.1, aes(x=longitude, y=latitude, 
                                         color=I('firebrick')), 
           data= filter(room_id_grouped, !is.na(price_bin))) +
  ggtitle("Location of units per price quartile") +
  scale_x_continuous(limits = c(-122.525, -122.3), 
                     expand = c(0, 0), breaks = NULL) +
  scale_y_continuous(limits = c(37.695, 37.84), 
                     expand = c(0, 0), breaks = NULL) +
  theme(axis.title = element_blank()) +
  facet_wrap(~price_bin, nrow=2)
...
### Description Three
The first price quartile has a much wider distribution compared to the rest. For 
the fourth price quartile the units are densely located in the more central 
parts of the city.
...
Exploring the Airbnb data from San Francisco has been an interesting endeavor. 
Although I do not think I have discovered any dramatic new findings, there are 
plenty of interesting observations to be made. As an example, I find it 
interesting how price has stayed fairly stagnant throughout the four years, 
while the amount of rental units on the market have increased dramatically.

Since I chose to find a dataset on my own I had to be extra wary of potential 
data quality issues. Several times throughout my exploratory analysis I came 
across suspicious values, dips and peaks. This led to lengthy examinations, 
but in the end I felt more confident about what parts of the data were 
trustworthy, and which variables were too uncertain to continue focusing on.

The clearest relationships in the dataset are also the most obvious ones, namely 
the correlation between price and the proxies for rental unit size (bedroom count 
and how many people the unit can accommodate). There is also a clear correlation 
...
and to see if any of the variables for individual housing units ever change.
