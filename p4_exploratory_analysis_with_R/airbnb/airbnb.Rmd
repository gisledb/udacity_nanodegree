...
Private rooms first have become a little bit cheaper, while prices for shared rooms, 
excluding one date in August 2015, have been fairly stable.
...
[unclear if keep or remove] - probably remove. Don't understand it.
...
```{r}
ggmap(myMap) +
    geom_jitter(size = 0.1, alpha = 0.1, aes(x=longitude, y=latitude), 
...
ggmap(myMap) +
geom_jitter(alpha = 0.1, size = 0.5, aes(x=longitude, y=latitude), 
           data= room_id_grouped) +
  ggtitle("Location of units per price quartiles") +
  facet_wrap(~price_bin)
...
year_grouped <- group_by(filter(airbnb, price < quantile(airbnb$price, 0.98)), 
                         room_id, longitude, latitude, price_bin, 
                         year=format(date_collected,"%Y")) %>% 
  summarise(n = n())
...
ggmap(myMap) +
geom_point(alpha = 0.1,size = 0.1,aes(x=longitude, y=latitude), data=year_grouped) +
  ...
  ggtitle("Location of units per year, with price range") +
...
plot3 <- ggmap(myMap) +
geom_jitter(alpha = 0.1, size = 0.1, aes(x=longitude, y=latitude, color=I('firebrick')), 
           data= filter(room_id_grouped, !is.na(price_bin))) +
  ggtitle("Location of units per price quartile") +
  scale_x_continuous(limits = c(-122.525, -122.3), expand = c(0, 0), breaks = NULL) +
  scale_y_continuous(limits = c(37.695, 37.84), expand = c(0, 0), breaks = NULL) +
...
data quality issues. Several times throughout my exploratory analysis I came across 
suspicious values, dips and peaks. This led to lengthy examinations, but in the 
end I felt more confident about what parts of the data were trustworthy, and 
which variables were too uncertain to continue focusing on.
