...
airbnb %>% 
  ...
  summarise(n = n(), price = mean(price), med = median(price), min = min(price), max = max(price)) %>% 
  arrange(desc(avg))
```
...
District has the most expensive units. Let's see if looking at median instead of 
mean changes things. One thing to note is that this plot excludes any units 
with zero bedrooms  (roughly 13% of the records). 
