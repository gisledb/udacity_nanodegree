...
head(characters)
str(characters)
movies <- read.csv('data/meta_data7.csv')
head(movies)
str(movies)
```
...
nrow(filter(inner_join(script_summary, movies, by = 'script_id',
...
Therefore I will rename some of the older award names in the combined.1995.to.2015 dataframe to the new 
award names, in the following way:
- 'Writing (Screenplay Based on Material Previously Produced or Published)' will be replaced by 'Writing (Adapted Screenplay)'
- 'Writing (Screenplay Written Directly for the Screen)' will
be replaced by 'Writing (Original Screenplay)'
-'Music (Original Dramatic Score)' and 'Music (Original Musical or Comedy Score)' will be replaced by 'Music (Original Score)'
...
award_replacements <- c(
 ...
 'Sound'='Sound Mixing',
 'Makeup'='Makeup and Hair Styling')

award_replacements

award_replacements[Sound]

count = 0

tmp_l <- for (name in names(award_replacements)) {
  count = count + 1
  `if`(combined.1995.to.2015$Award == name,
         award_replacements[count],
         ,combined.1995.to.2015$Award)
}

tmp_l

`if`(combined.1995.to.2015$Award == 'Writing (Screenplay Based on Material Previously Produced or Published)', 'Writing (Adapted Screenplay)', combined.1995.to.2015$Award)
```
...
script_summary <- group_by(characters, script_id, title) %>% 
  summarise(total_words = sum(words), characters = n()) %>% 
  arrange(desc(total_words))

script_summary %>% arrange(desc(characters))
